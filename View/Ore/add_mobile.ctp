...
<div class="row" id="ore-app">
    <b-col cols="12" v-if="personaId == ''" class="pt-3">
        ...
        <a href="<?= $this->Html->url(['controller'=> 'users', 'action'=>'logout']) ?>"
        ...
        <template v-if="!riepilogo">
            <b-row>
                <b-col cols="12" class="mt-2">
                    ...
                    <div class="mt-1" cols="12" v-if="selecAtt!=null"><i class="fas fa-bookmark"></i> attivita:
                        <strong>{{ allattivita[selecAtt] }}</strong></div>
                    ...
                    <div v-if="dettagli != null">{{dettagli}}</div>
                      <map-app 
                        v-if="locationStart!=null & locationStart!=''" 
                        :location-start="locationStart"
                        :location-stop="locationStop"
                        >
                      </map-app>
            ...
            <b-row v-if="selecFase == null">
                <b-col cols="6">
                    <b-form-input id="input-cerca1" v-model="cerca" class="m-3" placeholder="Cerca..."
                        aria-label="Cerca" class="w-50"></b-form-input>
                </b-col>
                <b-col cols="6">
                    <b-button v-if="selecAtt==null" pill id="btnattivita" @click="toggleattivita()" variant="info"
                        size="sm" class="mt-3">Vedi tutte le attivita</b-button>
                </b-col>
                <b-col cols="12" v-if="selecAtt==null">
                    <b-card no-body border-variant="light">
                        <b-card-header>{{titoloAttivita}}</b-card-header>
                        <b-form-radio-group id="btn-radios-3" v-model="selecAtt" class="border" v-on:change="getFasi"
                            :options="filteredAttvità" value-field="item" text-field="name"
                            button-variant="outline-secondary" buttons stacked name="radio-btn-stacked" class="w-100">
                        </b-form-radio-group>
                    </b-card>
                </b-col>
                <b-col cols="12" v-if="selecAtt!=null & selecFase == null">
                    <b-card no-body border-variant="light">
                        ...
                        <b-form-radio-group id="btn-radios-3" v-model="selecFase" class="border" value-field="item"
                            text-field="name" :options="filteredFase" button-variant="outline-secondary" buttons stacked
                            name="radio-btn-stacked" class="w-100"></b-form-radio-group>
                    </b-card>
                </b-col>
            </b-row>
            ...
            <b-row v-if="dateTimeStart==null && selecFase != null">
                ...
                <b-col cols="12" class="mt-5 text-center">
                    <b-button pill id="btnStop" @click="setStart()" variant="outline-success"

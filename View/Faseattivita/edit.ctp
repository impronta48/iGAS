...
<div class="faseattivita form">
		<legend><?php echo __('Edit Fase Attivita'); ?></legend>        
            ...
            <?php echo $this->Form->hidden('old_venduto', ['default' => $this->request->data['Faseattivita']['vendutou']]);?>
            <?php echo $this->Form->hidden('old_qta', ['default' => $this->request->data['Faseattivita']['qta']]);?>
            <?php echo $this->Form->input('attivita_id', ['class'=>'chosen-select']);?>
            <?php echo $this->Form->input('entrata', ['options' => [0 =>'Uscita', 1 => 'Entrata']] );?>
            ...
            foreach(Configure::read('iGas.commonFiles') as $ext => $mimes){
                if(file_exists(WWW_ROOT.'files'.DS.strtolower($this->request->controller).DS.$aid.'_'.$id.'.'.$ext)){
                    ...
                    echo $this->Html->link(__('View Attachment'), HTTP_BASE.DS.APP_DIR.DS.'files'.DS.$this->request->controller.DS.$aid.'_'.$id.'.'.$ext, ['title'=>'View related Attachment','class'=>'btn btn-xs btn-primary']);
                    echo '&nbsp;'; // Uso questo anche se non è bello perchè vedo che ogni tanto è già usato.
                    echo $this->Html->link(__('Delete Attachment'), ['action' => 'deleteDoc', $aid, $id], ['title'=>'Delete related Attachment','class'=>'btn btn-xs btn-primary'], __('Are you sure you want to delete %s_preventivo.%s?', $id, $ext));
            ...
            <div class="col col-md-offset-3">
                <?php echo  $this->Form->button('Salva',['class'=>'btn btn-primary', ]); ?>
            </div>

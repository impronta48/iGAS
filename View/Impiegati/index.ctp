<div class="impiegati index">
	...
	if (isset($persona_id)) {
		if (!empty($impiegati)) {
			echo $this->Html->link('<span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;' . __('Crea una variazione per questo impiegato'), ['action' => 'variation', $impiegati[0]['Impiegato']['id']], ['escape' => false, 'class' => 'btn btn-primary']);
		} else {
			echo $this->Html->link('<span class="glyphicon glyphicon-plus"></span>&nbsp;&nbsp;' . __('Crea una variazione per questo impiegato'), ['action' => 'add', $persona_id], ['escape' => false, 'class' => 'btn btn-primary']);
		}
	}
	?>
		...
		<tbody>
			...
			?>
				<tr class="<?php echo $extraclass ?>">
					<td>
						<?php echo $this->Html->link($impiegato['Persona']['DisplayName'], ['controller' => 'impiegati', 'action' => 'index', $impiegato['Persona']['id']]); ?>
						<?php if ($impiegato['Impiegato']['disattivo']) : ?>
							<span class="label label-danger">Disattivo</span>
						<?php endif; ?>
					...
					<td class="actions">
						<?php echo $this->Html->link('<span class="glyphicon glyphicon-user"></span>', ['controller' => 'persone', 'action' => 'edit', $impiegato['Persona']['id']], ['escape' => false]); ?>
						<?php echo $this->Html->link('<span class="glyphicon glyphicon-search"></span>', ['action' => 'view', $impiegato['Impiegato']['id']], ['escape' => false]); ?>
						<?php echo $this->Html->link('<span class="glyphicon glyphicon-edit"></span>', ['action' => 'edit', $impiegato['Impiegato']['id']], ['escape' => false]); ?>
						<?php echo $this->Form->postLink('<span class="glyphicon glyphicon-remove"></span>', ['action' => 'delete', $impiegato['Impiegato']['id']], ['escape' => false], __('Are you sure you want to delete # %s?', $impiegato['Impiegato']['persona_id'])); ?>
					</td>
				</tr>
			<?php endforeach; ?>
		</tbody>

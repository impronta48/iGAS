			...
			foreach($keys as $filename)
			{
				...
				else
				{
					...
					else
					{
                        ...
                        if ($this->Notaspesa->deleteAll(array(
								'eRisorsa' => $o['eRisorsa'],
...
//							if(empty($duplicate))
   ...
   public function edit($id) {
    ...
    public function box() {
        ...
        $persone = $this->Ora->find('all', array('fields' => array('DISTINCT Persona.id', 'Persona.Cognome', 'Persona.Nome'),
                                    'joins' => array(array('table' => 'notaspese',
                                    'alias' => 'Notaspesa',
                                    'type' => 'INNER'))));

        $ore = $this->Ora->find('all', array('fields' => array('SUM(Ora.numOre) as S', 'eRisorsa', 'data'),
                                             'group' => 'Ora.eRisorsa',
                                             'conditions' => array('Ora.data >=' => "$anno-$mese-01")));
        $this->loadModel('Notaspesa');

        $spese = $this->Notaspesa->find('all', array('fields' => array('SUM(Notaspesa.importo) as S', 'Notaspesa.eRisorsa'),
    ...
    public function totali() {
        ...
        $totale['Ore'] = $this->Ora->find('all', array('fields' => array('SUM(Ora.numOre) as S'),
                                             'conditions' => array('Ora.data >=' => "$anno-01-01")));

        $this->loadModel('Notaspesa');

        $totale['Spese'] = $this->Notaspesa->find('all', array('fields' => array('SUM(Notaspesa.importo) as S'),
...
?>
>>>>>>> origin/aggiuntaLinkPerCaricareOreInMailDiSollecito

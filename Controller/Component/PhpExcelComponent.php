...
class PhpExcelComponent extends Component {
     ...
     */
    /**
     * MarcoT: aggiunto parametro forceString altrimenti phpexcel toglie gli 0 iniziali dai numeri di telefono
     */
    public function addTableRow($data, $forceString=false) {
        $offset = $this->_tableParams['offset'];

        if($forceString) {
			foreach ($data as $d) {
				$type = PHPExcel_Cell_DataType::TYPE_STRING;
				$this->_xls->getActiveSheet()->getCellByColumnAndRow($offset++, $this->_row)->setValueExplicit($d, $type);
			}
		}
		else {
			foreach ($data as $d) {
